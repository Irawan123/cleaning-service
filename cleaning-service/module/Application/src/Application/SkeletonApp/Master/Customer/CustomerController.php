...
class CustomerController extends AbstractMainController
{
    ...
    public function findRequestedPegawai()
    {
        ...
        $model = $this->em->find('Application\SkeletonApp\Domain\Master\Customer', $id);

        return $model;
    }

    public function modelToArray(Customer $model)
    {
        $hydrator = new DoctrineObject(
            $this->em,
            'Application\SkeletonApp\Domain\Master\Customer'
        );
    ...
    {
        $model = $e->getParams();
        $message = 'Customer ' . $model['nama'] . ' berhasil disimpan.';
    ...
    {
        $model = $e->getParams();
        $message = 'Customer ' . $model['nama'] . ' berhasil dihapus.';
        $this->flashMessenger()->addSuccessMessage($message);
